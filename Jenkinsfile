pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/etoko-folio"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Simple-Etoko-Portfolio-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop etoko-folio | true"
                sh "docker rm etoko-folio | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name etoko-folio -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service etoko-folio-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"etoko-folio\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
